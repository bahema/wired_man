...
export default function ItemsPage() {
  ...
  const loadProducts = useCallback(async (silent = false) => {
    ...
    try {
      const data = await publicApi.fetchProducts({ placement: 'items' });
      ...
      if (mountedRef.current) {
        ...
        if (import.meta.env.DEV) {
          console.warn(
            ...
            buildApiUrl('/api/public/products?placement=items'),
            error
          );
        }
      }
  ...
  return (
    <Layout>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-semibold text-slate-900 sm:text-4xl md:mt-4 md:text-5xl dark:text-slate-100">
              Your first $2,000 starts with the right toolkit.
            </h1>
            <p className="mt-3 text-base text-slate-600 dark:text-slate-300 md:mt-4 md:text-lg">
              Explore automation resources, affiliate courses, and ready-to-launch packs.
            </p>
            <div className="mt-6 flex gap-3">
              <Button size="lg" onClick={() => open({ source: 'items-hero' })}>
                Subscribe for updates
              </Button>
              <Button size="lg" variant="secondary" onClick={() => open({ source: 'items-hero' })}>
                Daily offers
              </Button>
            </div>
          </div>
          <Card className="p-6">
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
              Get a curated email each week with new launches and discounts.
            </p>
            <Button className="mt-4" onClick={() => open({ source: 'items-highlight' })}>
              Join the list
            </Button>
          </Card>
        </div>
      ...
      <ClientPageSections pageKey="items" />

      <Section className="pt-0">
        {loading ? (
          <div className="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <Card key={`items-skeleton-${index}`} className="p-5 animate-pulse">
            ...
            <Card key={product.id} className="overflow-hidden">
              ...
              <div className="p-5">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    openAffiliateLink(product, 'items') || open({ source: 'items' })
                  }
                >
