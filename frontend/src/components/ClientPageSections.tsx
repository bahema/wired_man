...
import { ClientSection, publicApi } from '../services/publicApi';

type ClientPageSectionsProps = {
  pageKey: ClientSection['pageKey'];
};

const toArray = (value: any) => (Array.isArray(value) ? value : []);

export default function ClientPageSections({ pageKey }: ClientPageSectionsProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await publicApi.fetchClientSections(pageKey);
        if (active) {
          setSections(data);
        }
      } catch {
    ...
    const handler = (event: MessageEvent) => {
      try {
        ...
        if (changed.includes('client-sections')) {
          void load();
        }
      } catch {
  ...
  const renderSection = (section: ClientSection) => {
